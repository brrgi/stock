"""
날짜 선택 가능한 인터랙티브 대시보드 생성
JavaScript로 weekly_data/*.json 파일을 동적으로 로드
"""
...
def get_available_dates():
    """weekly_data 폴더에서 사용 가능한 날짜 목록"""
    if not os.path.exists(DATA_DIR):
        return []

    files = [f for f in os.listdir(DATA_DIR) if f.startswith('signals_') and f.endswith('.json')]
    dates = [f.replace('signals_', '').replace('.json', '') for f in files]
    return sorted(dates, reverse=True)


def generate_interactive_dashboard():
    """날짜 선택 기능이 있는 대시보드 생성"""

    dates = get_available_dates()
    if not dates:
        print("weekly_data 폴더에 데이터가 없습니다.")
        print("먼저 generate_weekly_data.py를 실행하세요.")
        return
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        @media (max-width: 480px) {{
            ...
            .header h1 {{ font-size: 0.95em; margin-bottom: 4px; }}
        ...
        @media (max-width: 380px) {{
            ...
            .header .subtitle {{ font-size: 0.66em; }}
            .date-selector {{ padding: 5px 6px; }}
            .date-selector select {{ padding: 5px 6px; font-size: 0.74em; }}
            .filters {{ padding: 5px 6px; gap: 3px; }}
            .filter-row select,
            .filter-row input {{ padding: 4px 6px; font-size: 0.72em; }}
        }}
...
"""
    ...
    output_path = os.path.join(WEB_DIR, 'dashboard_interactive.html')
...
if __name__ == "__main__":
    output = generate_interactive_dashboard()
    if output:

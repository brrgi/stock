...
def load_name_overrides():
    """종목명 보정 맵 로드 (있으면 우선 적용)"""
    overrides = {}
    override_paths = []
    override_paths.extend(glob.glob(os.path.join('results', '*.csv')))
    override_paths.append(os.path.join(DATA_DIR, 'name_overrides.csv'))
    override_paths.append('name_overrides.csv')
...
def load_krx_name_map():
    ...
    try:
        ...
        if not tables:
            return {}
        df = tables[0]
        if '종목코드' not in df.columns or '회사명' not in df.columns:
            return {}
        mapping = {}
...
def main():
    ...
    krx_names = load_krx_name_map()
    if krx_names:
        name_map.update(krx_names)
    if name_overrides:
    ...
    latest_path = os.path.join(DATA_DIR, 'latest_prices.json')
    ...
    for idx, date in enumerate(target_dates, 1):
        ...
        filename = os.path.join(DATA_DIR, f'signals_{date_str}.json')
